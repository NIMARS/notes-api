...
describe('auth for write routes', () => {
  //GET /health → 200.
  ...
  it('PATCH /notes/:id change title → 200', async () => {
    const note1 = await prisma.note.create({
      data: { title: 'Note 1', content: 'Content 1', tags: ['a'] },
    });
  ...
  it('PATCH /notes/:id not found → 404', async () => {
    const res = await request(app.server)
  ...
  it('GET /notes filter tagsAny=["a"]', async () => {
    ...
    const note2 = await prisma.note.create({
      data: { title: 'Note 2', content: 'Content 2', tags: ['a', 'b'] },
    });
    ...
    expect(res.body.items.map((n: Note) => n.id)).toEqual(
      expect.arrayContaining([note1.id, note2.id]),
    );
  ...
  it('GET /notes filter tagsAny+tagsAll', async () => {
    ...
    const note3 = await prisma.note.create({
      data: { title: 'Note 3', content: 'Content 3', tags: ['b'] },
    });
    const res = await request(app.server)
      .get('/notes')
      .query({ tagsAny: ['a', 'b'], tagsAll: ['b'] });
    expect(res.body.items.map((n: Note) => n.id)).toEqual(
      expect.arrayContaining([note2.id, note3.id]),
    );
  });
  ...
  it('Pagination: second page by nextCursor', async () => {
    ...
    expect([0, 1, 2]).toContain(second.body.items.length);
  });
  ...
  it('error - uniq JSON for ( error,message)', async () => {
    const res = await request(app.server).post('/notes').send({ title: '', content: '' });
    expect(res.status).toBe(400);
    expect(res.body).toHaveProperty('error');
    expect(res.body).toHaveProperty('message');
  });

  it('GET /v1/notes works like /notes', async () => {
    await prisma.note.create({ data: { title: 'A', content: 'B', tags: ['x'] } });

    const legacy = await request(app.server).get('/notes');
    const v1 = await request(app.server).get('/v1/notes');

    expect(v1.status).toBe(200);
    expect(v1.body.items).toHaveLength(legacy.body.items.length);
  });

  it('GET /notes returns deprecation headers', async () => {
    const res = await request(app.server).get('/notes');
